...
module.exports = {
    ...
    autoLinkifyHeaders: function(content) {
        return content.replace(/<h([1-6])>(.+?)<\/h[1-6]>/igm, function(match, type, text) {
            ...
            return "<h" + type + " id='" + makeSlug(link.substr(1)) + "'><a class='heading_anchor' href='"+link+"'><i class='headerLinkIcon'></i></a>"+text+"</h" + type + ">";
        });
    },
    ...
    headingTable: function (toh, maxLevel, classes) {
        if (toh.length > 1) {
            ...
            for (var h = 1; h < toh.length; h++) {
                ...
                tocHTML += "<li" + (classes.length ? " class='" + classes[1] + " level_" + assignedLevel + "'>" : ">") + "<a href='" + makeSlug(heading.link) + "'>" + heading.name + "</a>";
            }
